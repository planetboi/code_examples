...
class ServiceIf {
 public:
  virtual ~ServiceIf() {}
  virtual void seeProducts(std::vector<Product> & _return, const std::string& warehouse) = 0;
  virtual void orderProduct(std::string& _return, const std::string& product, const int32_t amount, const std::string& warehouse) = 0;
};
...
class ServiceNull : virtual public ServiceIf {
 public:
  virtual ~ServiceNull() {}
  void seeProducts(std::vector<Product> & /* _return */, const std::string& /* warehouse */) override {
    return;
  }
  void orderProduct(std::string& /* _return */, const std::string& /* product */, const int32_t /* amount */, const std::string& /* warehouse */) override {
    return;
  }
};

typedef struct _Service_seeProducts_args__isset {
  _Service_seeProducts_args__isset() : warehouse(false) {}
  bool warehouse :1;
} _Service_seeProducts_args__isset;

class Service_seeProducts_args {
 public:

  Service_seeProducts_args(const Service_seeProducts_args&);
  Service_seeProducts_args& operator=(const Service_seeProducts_args&);
...
typedef struct _Service_orderProduct_args__isset {
  _Service_orderProduct_args__isset() : product(false), amount(false), warehouse(false) {}
  bool product :1;
  bool amount :1;
  bool warehouse :1;
...
class ServiceClient : virtual public ServiceIf {
 ...
 public:
  ...
  void seeProducts(std::vector<Product> & _return, const std::string& warehouse) override;
  void send_seeProducts(const std::string& warehouse);
  void recv_seeProducts(std::vector<Product> & _return);
  void orderProduct(std::string& _return, const std::string& product, const int32_t amount, const std::string& warehouse) override;
  void send_orderProduct(const std::string& product, const int32_t amount, const std::string& warehouse);
...
class ServiceMultiface : virtual public ServiceIf {
 ...
 public:
  void seeProducts(std::vector<Product> & _return, const std::string& warehouse) override {
    ...
    size_t i = 0;
    for (; i < (sz - 1); ++i) {
      ifaces_[i]->seeProducts(_return, warehouse);
    }
    ifaces_[i]->seeProducts(_return, warehouse);
    return;
  }
  ...
  void orderProduct(std::string& _return, const std::string& product, const int32_t amount, const std::string& warehouse) override {
    ...
    size_t i = 0;
    for (; i < (sz - 1); ++i) {
      ifaces_[i]->orderProduct(_return, product, amount, warehouse);
    }
    ifaces_[i]->orderProduct(_return, product, amount, warehouse);
    return;
  }
  ...
  ServiceConcurrentClient(std::shared_ptr< ::apache::thrift::protocol::TProtocol> iprot, std::shared_ptr< ::apache::thrift::protocol::TProtocol> oprot, std::shared_ptr<::apache::thrift::async::TConcurrentClientSyncInfo> sync) : sync_(sync)
{
 ...
 public:
  ...
  void seeProducts(std::vector<Product> & _return, const std::string& warehouse) override;
  int32_t send_seeProducts(const std::string& warehouse);
  void recv_seeProducts(std::vector<Product> & _return, const int32_t seqid);
  void orderProduct(std::string& _return, const std::string& product, const int32_t amount, const std::string& warehouse) override;
  int32_t send_orderProduct(const std::string& product, const int32_t amount, const std::string& warehouse);
